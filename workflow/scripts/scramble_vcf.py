...
def write_vcf_header(sample_name):
    ...
    vcf_out.write("##INFO=<ID=SVLEN,Number=.,Type=Integer,Description=\"Difference in length between REF and ALT alleles\">\n")
    vcf_out.write("##INFO=<ID=SVTYPE,Number=1,Type=String,Description=\"Type of structural variant\">\n")
    vcf_out.write("##INFO=<ID=MEINFO,Number=4,Type=String,Description=\"Mobile element info of the form NAME,START,END,POLARITY\">\n")
    vcf_out.write("##INFO=<ID=CALLER,Number=1,Type=String,Description=\"Caller\">\n")
    vcf_out.write("##INFO=<ID=SUPPORT,Number=1,Type=Integer,Description=\"Number of supporting reads\">\n")
    vcf_out.write("##INFO=<ID=SCORE,Number=1,Type=Float,Description=\"SCRAMBLE alignment score\">\n")
    vcf_out.write("##INFO=<ID=CLIP_SIDE,Number=1,Type=String,Description=\"Side of soft-clipping (left/right)\">\n")
    vcf_out.write("##INFO=<ID=POLYA_LEN,Number=1,Type=Integer,Description=\"Length of polyA tail\">\n")
    vcf_out.write("##INFO=<ID=POLYA_SCORE,Number=1,Type=Float,Description=\"PolyA alignment score\">\n")
    vcf_out.write("##ALT=<ID=INS:ME:ALU,Description=\"Insertion of ALU element\">\n")
    vcf_out.write("##ALT=<ID=INS:ME:L1,Description=\"Insertion of L1 element\">\n")
    vcf_out.write("##ALT=<ID=INS:ME:SVA,Description=\"Insertion of SVA element\">\n")
