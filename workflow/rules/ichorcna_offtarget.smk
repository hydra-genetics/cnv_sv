...
rule ichorcna_offtarget_read_counter:
    ...
    benchmark:
        repeat(
            ...
            config.get("ichorcna_offtarget_read_counter", {}).get(
                "benchmark_repeats", 1
            ),
        )
    threads: config.get("ichorcna_offtarget_read_counter", {}).get("threads", config["default_resources"]["threads"])
    resources:
        mem_mb=config.get("ichorcna_offtarget_read_counter", {}).get(
            "mem_mb", config["default_resources"]["mem_mb"]
        ),
        mem_per_cpu=config.get("ichorcna_offtarget_read_counter", {}).get(
            "mem_per_cpu", config["default_resources"]["mem_per_cpu"]
        ),
        partition=config.get("ichorcna_offtarget_read_counter", {}).get(
            "partition", config["default_resources"]["partition"]
        ),
        threads=config.get("ichorcna_offtarget_read_counter", {}).get(
            "threads", config["default_resources"]["threads"]
        ),
        time=config.get("ichorcna_offtarget_read_counter", {}).get(
...
rule ichorcna_offtarget_run:
    ...
    output:
        cna=temp(
            "cnv_sv/ichorcna_offtarget_run/{sample}_{type}/{sample}_{type}.cna.seg"
        ),
        seg=temp("cnv_sv/ichorcna_offtarget_run/{sample}_{type}/{sample}_{type}.seg"),
        seg_txt=temp(
            "cnv_sv/ichorcna_offtarget_run/{sample}_{type}/{sample}_{type}.seg.txt"
        ),
        params_txt=temp(
            "cnv_sv/ichorcna_offtarget_run/{sample}_{type}/{sample}_{type}.params.txt"
        ),
        corrected_depth=temp(
            "cnv_sv/ichorcna_offtarget_run/{sample}_{type}/{sample}_{type}.correctedDepth.txt"
        ),
        rdata=temp(
            "cnv_sv/ichorcna_offtarget_run/{sample}_{type}/{sample}_{type}.RData"
        ),
        plot_dir=temp(
            directory("cnv_sv/ichorcna_offtarget_run/{sample}_{type}/{sample}_{type}/")
        ),
    params:
        ...
        include_homd=config.get("ichorcna_offtarget_run", {}).get(
            "include_homd", "FALSE"
        ),
        chrs=config.get("ichorcna_offtarget_run", {}).get("chrs", 'c(1:22,"X")'),
        chr_train=config.get("ichorcna_offtarget_run", {}).get("chr_train", "c(1:22)"),
        genome_build=config.get("ichorcna_offtarget_run", {}).get(
            "genome_build", "hg38"
        ),
        genome_style=config.get("ichorcna_offtarget_run", {}).get(
            "genome_style", "UCSC"
        ),
    ...
    resources:
        mem_mb=config.get("ichorcna_offtarget_run", {}).get(
            "mem_mb", config["default_resources"]["mem_mb"]
        ),
        mem_per_cpu=config.get("ichorcna_offtarget_run", {}).get(
            "mem_per_cpu", config["default_resources"]["mem_per_cpu"]
        ),
        partition=config.get("ichorcna_offtarget_run", {}).get(
            "partition", config["default_resources"]["partition"]
        ),
        threads=config.get("ichorcna_offtarget_run", {}).get(
            "threads", config["default_resources"]["threads"]
        ),
        time=config.get("ichorcna_offtarget_run", {}).get(
            "time", config["default_resources"]["time"]
        ),
    container:
        config.get("ichorcna_offtarget_run", {}).get(
            "container", config["default_container"]
        )
    message:
