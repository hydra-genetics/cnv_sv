...
rule pbsv_discover:
    ...
    shell:
        "pbsv discover " "--tandem-repeats {params.trf} " "--log-file {log.discover} " "{input.bam} {output.svsig}"
...
rule pbsv_call:
    ...
    shell:
        "pbsv call -j {threads} " "--log-file {log.call} " "--hifi {input.ref} " "{input.svsig} " "{output.vcf}"
