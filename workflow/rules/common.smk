...
def compile_output_list(wildcards):
    output_files = [
        "cnv_sv/cnvkit_call_loh/%s_%s.loh.cns" % (sample, t)
    ...
    output_files.append(
        ["cnv_sv/svdb_merge/%s_%s.merged.vcf" % (sample, t) for sample in get_samples(samples) for t in get_unit_types(units, sample)]
    )
    return output_files
