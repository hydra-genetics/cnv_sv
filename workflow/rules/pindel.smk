...
rule pindel2vcf:
    input:
        ...
        ref=config["reference"]["fasta"],
    output:
        vcf=temp("cnv_sv/pindel/{sample}.noContig.vcf"),
    params:
    ...
    log:
        "cnv_sv/pindel/{sample}.noContig.vcf.log",
    benchmark:
        repeat(
            "cnv_sv/pindel/{sample}.noContig.vcf.benchmark.tsv",
    ...
    conda:
        "../envs/pindel.yaml"
    message:
        "{rule}: Convert pindel output to vcf for {wildcards.sample}.noContig"
    wrapper:
