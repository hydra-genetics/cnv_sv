...
rule hificnv:
    ...
    shell:
        ...
        "&> {log.call} && "
        "mv $PREFIX.*.vcf.gz $PREFIX'.vcf.gz' &> {log.mv_vcf} && "
        "mv $PREFIX.*.depth.bw $PREFIX'.depth.bw' &> {log.mv_bw} && "
        "mv $PREFIX.*.copynum.bedgraph $PREFIX'.copynum.bedgraph' &> {log.mv_bedgraph}"
