...
rule hificnv:
    ...
    shell:
        ...
        "&> {log.call} && "
        "mv $PREFIX.{wildcards.sample}_{wildcards.type}'.vcf.gz' $PREFIX'.vcf.gz' &> {log.mv_vcf} && "
        "mv $PREFIX.{wildcards.sample}_{wildcards.type}'.depth.bw' $PREFIX'.depth.bw' &> {log.mv_bw} && "
        "mv $PREFIX.{wildcards.sample}_{wildcards.type}'.copynum.bedgraph' $PREFIX'.copynum.bedgraph' &> {log.mv_bedgraph}"
